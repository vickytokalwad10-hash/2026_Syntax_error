...
export default function MarketsView() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <form onSubmit={runOptimizer} className="agri-card" style={{ padding: '22px 24px' }}>
        ...
        {/* Input Parameters Bar with clean white styling */}
        <div style={{ 
          display: 'grid', 
          ...
          <div>
            ...
            >
              ...
              <span>{loading ? 'Optimizing...' : t('recalcRealization')}</span>
            </button>
          </div>
        </div>
      </form>
      ...
      {result && result.optimal_mandi && (
        <>
          ...
          <div className="agri-card" style={{ borderLeft: '4px solid #D97706', padding: '22px 26px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '6px', fontSize: '0.82rem', color: '#64748B' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <MapPin size={13} color="#D97706" /> {result.optimal_mandi.district}, {result.optimal_mandi.state}
                  </span>
                  <span>•</span>
                  <span>Distance: <strong style={{ color: '#0F172A' }}>{result.optimal_mandi.distance_km} km</strong></span>
                  <span>•</span>
                  <span>Mandi Cess: <strong style={{ color: '#0F172A' }}>₹{result.optimal_mandi.cess_cost_per_q}/Q</strong></span>
                </div>
              </div>

              <div style={{ textAlign: 'right', background: '#FEF3C7', padding: '12px 20px', borderRadius: '6px', border: '1px solid #FCD34D' }}>
                <span style={{ fontSize: '0.72rem', color: '#92400E', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: '700' }}>
                  Total Estimated Net Payout
                </span>
                <div style={{ fontSize: '2rem', fontWeight: '800', color: '#92400E', lineHeight: '1.2' }}>
                  ₹{result.optimal_mandi.total_net_payout.toLocaleString()}
                </div>
                <div style={{ fontSize: '0.88rem', color: '#D97706', fontWeight: '700' }}>
                  ₹{result.optimal_mandi.net_realized_price_per_q.toLocaleString()} <span style={{ fontSize: '0.75rem', color: '#92400E' }}>/ Quintal Net</span>
                </div>
              </div>
            </div>
              ...
              <table className="agri-table">
                ...
                <tbody>
                  {result.mandi_arbitrage_rankings.map((m) => (
                    <tr key={m.mandi_id} style={{ background: m.is_optimal ? '#FEF3C7' : 'transparent' }}>
                      <td>
                        <strong style={{ color: '#0F172A' }}>{m.mandi_name}</strong>
                        <div style={{ fontSize: '0.72rem', color: '#64748B' }}>{m.district}, {m.state}</div>
                      </td>
                      ...
                      <td>
                        <span style={{ fontSize: '0.98rem', fontWeight: '800', color: m.is_optimal ? '#92400E' : '#0F172A' }}>
              ...
              <table className="agri-table">
                ...
                <tbody>
                  {result.sell_vs_store_matrix.map((row, idx) => (
                    <tr key={idx}>
                      <td>
                        <strong style={{ color: '#0F172A' }}>{row.horizon}</strong>
                      </td>
                      ...
                      <td>
                        <span style={{ color: row.net_gain_vs_now_per_q >= 0 ? '#059669' : '#DC2626', fontWeight: '700' }}>
                          {row.net_gain_vs_now_per_q >= 0 ? `+₹${row.net_gain_vs_now_per_q}` : `-₹${Math.abs(row.net_gain_vs_now_per_q)}`}
                        </span>
                      </td>
                      <td>
                        <span style={{ color: row.roi_pct >= 0 ? '#059669' : '#DC2626', fontWeight: '700' }}>
